...
enchant.AvatarBG = enchant.Class.create(enchant.Group, {
	...
	initialize:function(mode){
		...
		var game = enchant.Game.instance;	// ADDED to @usamyu(2012/04/29)

		this.veryfarbg = new Sprite(320,51);
		this.veryfarbg.y=0;
		this.veryfarbg.image = game.assets["http://jsrun.it/assets/3/0/f/E/30fEq.png"];
		this.veryfarbg.frame=mode;
		this.addChild(this.veryfarbg);
		this.farbgs = [];
		for(i=0;i<3;i++){
			var bg = new Sprite(320,32);
			bg.image = game.assets["http://jsrun.it/assets/s/Z/V/N/sZVNl.png"];
			bg.frame=mode;
		...
		for(i=0;i<14;i++){
			...
			tile.image = game.assets["http://jsrun.it/assets/h/I/Y/z/hIYzl.png"];
			tile.frame=mode;
